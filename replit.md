...
- **Multi-tenancy**: Dual-layer isolation: (1) EF Core `HasQueryFilter` on all 13 `ITenantEntity` entities filtering by `CurrentTenantId`, (2) PostgreSQL RLS with `FORCE ROW LEVEL SECURITY` + `TenantSessionInterceptor` setting `app.current_tenant`. `TenantHttpHandler` sends `X-Tenant-ID` header, `CurrentTenantService` reads it server-side. Data pages subscribe to `TenantState.OnChange` for reactive reload on tenant switch.
