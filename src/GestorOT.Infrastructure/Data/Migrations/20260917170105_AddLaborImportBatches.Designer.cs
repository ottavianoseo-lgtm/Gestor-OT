// <auto-generated />
using System;
using GestorOT.Infrastructure.Data;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using NetTopologySuite.Geometries;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;

#nullable disable

namespace GestorOT.Infrastructure.Data.Migrations
{
    [DbContext(typeof(ApplicationDbContext))]
    [Migration("20260917170105_AddLaborImportBatches")]
    partial class AddLaborImportBatches
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "10.0.0")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.HasPostgresExtension(modelBuilder, "postgis");
            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("GestorOT.Domain.Entities.AccountConfiguration", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<long?>("CodComprobante")
                        .HasColumnType("bigint");

                    b.Property<long?>("CodCuentaDebeAuxiliar")
                        .HasColumnType("bigint");

                    b.Property<long?>("CodCuentaDebeCentro")
                        .HasColumnType("bigint");

                    b.Property<long?>("CodCuentaDebeContabilidad")
                        .HasColumnType("bigint");

                    b.Property<long?>("CodCuentaDebeGestion")
                        .HasColumnType("bigint");

                    b.Property<long?>("CodCuentaHaberAuxiliar")
                        .HasColumnType("bigint");

                    b.Property<long?>("CodCuentaHaberCentro")
                        .HasColumnType("bigint");

                    b.Property<long?>("CodCuentaHaberContabilidad")
                        .HasColumnType("bigint");

                    b.Property<long?>("CodCuentaHaberGestion")
                        .HasColumnType("bigint");

                    b.Property<long?>("CodEmpresa")
                        .HasColumnType("bigint");

                    b.Property<long?>("CodMoneda")
                        .HasColumnType("bigint");

                    b.Property<long?>("CodPerfilDebe")
                        .HasColumnType("bigint");

                    b.Property<long?>("CodPerfilHaber")
                        .HasColumnType("bigint");

                    b.Property<long?>("CodPersona")
                        .HasColumnType("bigint");

                    b.Property<string>("CreditAccountCode")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("DebitAccountCode")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Description")
                        .HasColumnType("text");

                    b.Property<Guid?>("ErpActivityId")
                        .HasColumnType("uuid");

                    b.Property<int?>("ExecutionMode")
                        .HasColumnType("integer");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean");

                    b.Property<Guid?>("LaborTypeId")
                        .HasColumnType("uuid");

                    b.Property<bool>("NoImputaAuxiliar")
                        .HasColumnType("boolean");

                    b.Property<bool>("NoImputaCentro")
                        .HasColumnType("boolean");

                    b.Property<bool>("NoImputaContabilidad")
                        .HasColumnType("boolean");

                    b.Property<bool>("NoImputaGestion")
                        .HasColumnType("boolean");

                    b.Property<int>("PuntoVenta")
                        .HasColumnType("integer");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("ErpActivityId");

                    b.HasIndex("LaborTypeId");

                    b.ToTable("AccountConfigurations");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.AuditLog", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Action")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("EntityId")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("EntityType")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("NewValue")
                        .HasColumnType("text");

                    b.Property<string>("OldValue")
                        .HasColumnType("text");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("Timestamp")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<string>("UserEmail")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("UserId")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.HasKey("Id");

                    b.ToTable("AuditLogs", "public");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.Campaign", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<decimal>("BudgetTotalUSD")
                        .ValueGeneratedOnAdd()
                        .HasPrecision(18, 4)
                        .HasColumnType("numeric(18,4)")
                        .HasDefaultValue(0m);

                    b.Property<string>("BusinessRulesJson")
                        .HasColumnType("jsonb")
                        .HasColumnName("BusinessRules");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<DateOnly>("EndDate")
                        .HasColumnType("date");

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<DateOnly>("StartDate")
                        .HasColumnType("date");

                    b.Property<string>("Status")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasDefaultValue("Active");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.ToTable("Campaigns", "public");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.CampaignField", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<decimal>("AllocatedHectares")
                        .ValueGeneratedOnAdd()
                        .HasPrecision(18, 4)
                        .HasColumnType("numeric(18,4)")
                        .HasDefaultValue(0m);

                    b.Property<Guid>("CampaignId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("FieldId")
                        .HasColumnType("uuid");

                    b.Property<decimal>("TargetYieldTonHa")
                        .ValueGeneratedOnAdd()
                        .HasPrecision(18, 4)
                        .HasColumnType("numeric(18,4)")
                        .HasDefaultValue(0m);

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("FieldId");

                    b.HasIndex("CampaignId", "FieldId")
                        .IsUnique();

                    b.ToTable("CampaignFields", "public");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.CampaignLot", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<Guid>("CampaignId")
                        .HasColumnType("uuid");

                    b.Property<long?>("CodCentro")
                        .HasColumnType("bigint");

                    b.Property<Guid?>("CropId")
                        .HasColumnType("uuid");

                    b.Property<Geometry>("Geometry")
                        .HasColumnType("geometry(Geometry, 4326)");

                    b.Property<Guid>("LotId")
                        .HasColumnType("uuid");

                    b.Property<decimal>("ProductiveArea")
                        .HasPrecision(18, 4)
                        .HasColumnType("numeric(18,4)");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("LotId");

                    b.HasIndex("CampaignId", "LotId")
                        .IsUnique();

                    b.ToTable("CampaignLots", "public");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.Contact", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Email")
                        .HasColumnType("text");

                    b.Property<Guid?>("ErpPersonId")
                        .HasColumnType("uuid");

                    b.Property<string>("ExternalErpId")
                        .HasColumnType("text");

                    b.Property<string>("FullName")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("LegalName")
                        .HasColumnType("text");

                    b.Property<string>("Position")
                        .HasColumnType("text");

                    b.Property<int>("Role")
                        .HasColumnType("integer");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<string>("VatNumber")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("ErpPersonId");

                    b.ToTable("Contacts");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.CropStrategy", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<Guid?>("ErpActivityId")
                        .HasColumnType("uuid");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("ErpActivityId");

                    b.ToTable("CropStrategies", "public");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.ErpActivity", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("ExternalErpId")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.ToTable("ErpActivities", "public");

                    b.HasData(
                        new
                        {
                            Id = new Guid("5f96e4e0-0b6e-4f0e-8d8a-9f8e8e8e8e01"),
                            IsActive = true,
                            Name = "Trigo",
                            TenantId = new Guid("00000000-0000-0000-0000-000000000000")
                        },
                        new
                        {
                            Id = new Guid("5f96e4e0-0b6e-4f0e-8d8a-9f8e8e8e8e02"),
                            IsActive = true,
                            Name = "Sorgo",
                            TenantId = new Guid("00000000-0000-0000-0000-000000000000")
                        },
                        new
                        {
                            Id = new Guid("5f96e4e0-0b6e-4f0e-8d8a-9f8e8e8e8e03"),
                            IsActive = true,
                            Name = "Avena",
                            TenantId = new Guid("00000000-0000-0000-0000-000000000000")
                        },
                        new
                        {
                            Id = new Guid("5f96e4e0-0b6e-4f0e-8d8a-9f8e8e8e8e04"),
                            IsActive = true,
                            Name = "Maní",
                            TenantId = new Guid("00000000-0000-0000-0000-000000000000")
                        },
                        new
                        {
                            Id = new Guid("5f96e4e0-0b6e-4f0e-8d8a-9f8e8e8e8e05"),
                            IsActive = true,
                            Name = "Camelina",
                            TenantId = new Guid("00000000-0000-0000-0000-000000000000")
                        },
                        new
                        {
                            Id = new Guid("5f96e4e0-0b6e-4f0e-8d8a-9f8e8e8e8e06"),
                            IsActive = true,
                            Name = "Alpiste",
                            TenantId = new Guid("00000000-0000-0000-0000-000000000000")
                        },
                        new
                        {
                            Id = new Guid("5f96e4e0-0b6e-4f0e-8d8a-9f8e8e8e8e07"),
                            IsActive = true,
                            Name = "Girasol",
                            TenantId = new Guid("00000000-0000-0000-0000-000000000000")
                        },
                        new
                        {
                            Id = new Guid("5f96e4e0-0b6e-4f0e-8d8a-9f8e8e8e8e08"),
                            IsActive = true,
                            Name = "Girasol alto oleico",
                            TenantId = new Guid("00000000-0000-0000-0000-000000000000")
                        },
                        new
                        {
                            Id = new Guid("5f96e4e0-0b6e-4f0e-8d8a-9f8e8e8e8e09"),
                            IsActive = true,
                            Name = "Soja 1º",
                            TenantId = new Guid("00000000-0000-0000-0000-000000000000")
                        },
                        new
                        {
                            Id = new Guid("5f96e4e0-0b6e-4f0e-8d8a-9f8e8e8e8e10"),
                            IsActive = true,
                            Name = "Soja 2º",
                            TenantId = new Guid("00000000-0000-0000-0000-000000000000")
                        },
                        new
                        {
                            Id = new Guid("5f96e4e0-0b6e-4f0e-8d8a-9f8e8e8e8e11"),
                            IsActive = true,
                            Name = "Papa",
                            TenantId = new Guid("00000000-0000-0000-0000-000000000000")
                        },
                        new
                        {
                            Id = new Guid("5f96e4e0-0b6e-4f0e-8d8a-9f8e8e8e8e12"),
                            IsActive = true,
                            Name = "Cebada cervezera",
                            TenantId = new Guid("00000000-0000-0000-0000-000000000000")
                        },
                        new
                        {
                            Id = new Guid("5f96e4e0-0b6e-4f0e-8d8a-9f8e8e8e8e13"),
                            IsActive = true,
                            Name = "Cebada forrajera",
                            TenantId = new Guid("00000000-0000-0000-0000-000000000000")
                        },
                        new
                        {
                            Id = new Guid("5f96e4e0-0b6e-4f0e-8d8a-9f8e8e8e8e14"),
                            IsActive = true,
                            Name = "Maíz",
                            TenantId = new Guid("00000000-0000-0000-0000-000000000000")
                        },
                        new
                        {
                            Id = new Guid("5f96e4e0-0b6e-4f0e-8d8a-9f8e8e8e8e15"),
                            IsActive = true,
                            Name = "Maíz tardío",
                            TenantId = new Guid("00000000-0000-0000-0000-000000000000")
                        });
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.ErpConcept", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<int?>("ExecutionMode")
                        .HasColumnType("integer");

                    b.Property<string>("ExternalErpId")
                        .HasColumnType("text");

                    b.Property<string>("GrupoConcepto")
                        .HasColumnType("text");

                    b.Property<DateTime>("LastSyncDate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<double>("Stock")
                        .HasColumnType("double precision");

                    b.Property<string>("SubGrupoConcepto")
                        .HasColumnType("text");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<string>("UnitA")
                        .HasColumnType("text");

                    b.Property<string>("UnitB")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("ErpConcepts");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.ErpPerson", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Alias")
                        .HasColumnType("text");

                    b.Property<string>("Country")
                        .HasColumnType("text");

                    b.Property<string>("DocumentType")
                        .HasColumnType("text");

                    b.Property<bool>("Enabled")
                        .HasColumnType("boolean");

                    b.Property<string>("ExternalErpId")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("FullName")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Group")
                        .HasColumnType("text");

                    b.Property<bool>("IsActivated")
                        .HasColumnType("boolean");

                    b.Property<DateTime>("LastSyncDate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid?>("LinkedContactId")
                        .HasColumnType("uuid");

                    b.Property<string>("PersonType")
                        .HasColumnType("text");

                    b.Property<string>("ResponsibleTax")
                        .HasColumnType("text");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<string>("VatNumber")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("ErpPeople");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.Field", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<long?>("CodCentro")
                        .HasColumnType("bigint");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.ToTable("Fields", "public");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.FileAsset", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<byte[]>("Content")
                        .IsRequired()
                        .HasColumnType("bytea");

                    b.Property<string>("FileName")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<string>("Hash")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("MimeType")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<long>("SizeBytes")
                        .HasColumnType("bigint");

                    b.Property<string>("Tags")
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("UploadedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UploadedBy")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("Visibility")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.HasKey("Id");

                    b.ToTable("FileAssets", "public");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.Inventory", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Category")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<double>("ConversionFactor")
                        .ValueGeneratedOnAdd()
                        .HasPrecision(18, 6)
                        .HasColumnType("double precision")
                        .HasDefaultValue(1.0);

                    b.Property<double>("CurrentStock")
                        .HasPrecision(18, 4)
                        .HasColumnType("double precision");

                    b.Property<string>("ExternalErpId")
                        .HasColumnType("text");

                    b.Property<string>("GrupoConcepto")
                        .HasColumnType("text");

                    b.Property<string>("ItemName")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<double>("ReorderLevel")
                        .HasPrecision(18, 4)
                        .HasColumnType("double precision");

                    b.Property<string>("SubGrupoConcepto")
                        .HasColumnType("text");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<string>("Unit")
                        .HasColumnType("text");

                    b.Property<string>("UnitA")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("UnitB")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.HasKey("Id");

                    b.ToTable("Inventories", "public");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.Labor", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<Guid?>("AccountConfigurationId")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("CampaignLotId")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("ContactId")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<decimal>("EffectiveArea")
                        .HasPrecision(18, 4)
                        .HasColumnType("numeric(18,4)");

                    b.Property<Guid?>("ErpActivityId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("EstimatedDate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("EvidencePhotosJson")
                        .HasColumnType("text");

                    b.Property<DateTime?>("ExecutionDate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<decimal>("Hectares")
                        .HasPrecision(18, 4)
                        .HasColumnType("numeric(18,4)");

                    b.Property<bool>("IsExternalBilling")
                        .HasColumnType("boolean");

                    b.Property<bool>("IsOriginalPlan")
                        .HasColumnType("boolean");

                    b.Property<Guid>("LaborTypeId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("LotId")
                        .HasColumnType("uuid");

                    b.Property<string>("MachineryUsedId")
                        .HasColumnType("text");

                    b.Property<string>("MetadataExterna")
                        .HasColumnType("jsonb");

                    b.Property<string>("Mode")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasDefaultValue("Planned");

                    b.Property<string>("Notes")
                        .HasColumnType("text");

                    b.Property<decimal>("PlannedDose")
                        .HasColumnType("numeric");

                    b.Property<Guid?>("PlannedLaborId")
                        .HasColumnType("uuid");

                    b.Property<string>("PrescriptionMapUrl")
                        .HasColumnType("text");

                    b.Property<int>("Priority")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(1);

                    b.Property<decimal>("Rate")
                        .ValueGeneratedOnAdd()
                        .HasPrecision(18, 4)
                        .HasColumnType("numeric(18,4)")
                        .HasDefaultValue(0m);

                    b.Property<string>("RateUnit")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasDefaultValue("ha");

                    b.Property<decimal?>("RealizedDose")
                        .HasColumnType("numeric");

                    b.Property<Guid?>("SourceStrategyId")
                        .HasColumnType("uuid");

                    b.Property<string>("Status")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasDefaultValue("Planned");

                    b.Property<string>("SupplyWithdrawalNotes")
                        .HasColumnType("text");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<string>("WeatherLogJson")
                        .HasColumnType("text");

                    b.Property<Guid?>("WorkOrderId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("AccountConfigurationId");

                    b.HasIndex("CampaignLotId");

                    b.HasIndex("ContactId");

                    b.HasIndex("ErpActivityId");

                    b.HasIndex("LaborTypeId");

                    b.HasIndex("LotId");

                    b.HasIndex("PlannedLaborId");

                    b.HasIndex("SourceStrategyId");

                    b.HasIndex("WorkOrderId");

                    b.ToTable("Labors", "public");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.LaborAttachment", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<byte[]>("Content")
                        .IsRequired()
                        .HasColumnType("bytea");

                    b.Property<string>("FileName")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<long>("FileSizeBytes")
                        .HasColumnType("bigint");

                    b.Property<Guid>("LaborId")
                        .HasColumnType("uuid");

                    b.Property<string>("MimeType")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("UploadedAt")
                        .HasColumnType("timestamp with time zone");

                    b.HasKey("Id");

                    b.HasIndex("LaborId");

                    b.ToTable("LaborAttachments", "public");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.LaborFileAsset", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<Guid>("FileAssetId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("LaborId")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("LinkedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("FileAssetId");

                    b.HasIndex("LaborId", "FileAssetId")
                        .IsUnique();

                    b.ToTable("LaborFileAssets", "public");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.LaborImportBatch", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<Guid>("CampaignId")
                        .HasColumnType("uuid");

                    b.Property<int>("ExcludedCount")
                        .HasColumnType("integer");

                    b.Property<string>("FileHash")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("FileName")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<int>("ImportedCount")
                        .HasColumnType("integer");

                    b.Property<string>("LaborTypeMappingsJson")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<int>("PendingCount")
                        .HasColumnType("integer");

                    b.Property<int>("Status")
                        .HasColumnType("integer");

                    b.Property<string>("SupplierMappingsJson")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("SupplyMappingsJson")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<int>("TotalRows")
                        .HasColumnType("integer");

                    b.Property<DateTime>("UploadedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UploadedBy")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("CampaignId");

                    b.ToTable("LaborImportBatches");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.LaborImportPendingRow", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<Guid>("BatchId")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("CampaignLotId")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("ContactId")
                        .HasColumnType("uuid");

                    b.Property<string>("Contractor")
                        .HasColumnType("text");

                    b.Property<DateTime?>("Date")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("ErrorsJson")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("FieldName")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<decimal>("Hectares")
                        .HasColumnType("numeric");

                    b.Property<bool>("IsExternalBilling")
                        .HasColumnType("boolean");

                    b.Property<Guid?>("LaborTypeId")
                        .HasColumnType("uuid");

                    b.Property<string>("LaborTypeName")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid?>("LotId")
                        .HasColumnType("uuid");

                    b.Property<string>("LotName")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("MatchedContactName")
                        .HasColumnType("text");

                    b.Property<int>("Resolution")
                        .HasColumnType("integer");

                    b.Property<DateTime?>("ResolvedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid?>("ResultLaborId")
                        .HasColumnType("uuid");

                    b.Property<int>("RowIndex")
                        .HasColumnType("integer");

                    b.Property<string>("SuppliesJson")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<string>("WarningsJson")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("BatchId");

                    b.ToTable("LaborImportPendingRows");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.LaborSupply", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<decimal?>("CalculatedDose")
                        .HasPrecision(18, 6)
                        .HasColumnType("numeric(18,6)");

                    b.Property<decimal?>("CalculatedTotal")
                        .HasPrecision(18, 4)
                        .HasColumnType("numeric(18,4)");

                    b.Property<bool>("IsSubstitute")
                        .HasColumnType("boolean");

                    b.Property<Guid>("LaborId")
                        .HasColumnType("uuid");

                    b.Property<decimal>("PlannedDose")
                        .HasPrecision(18, 6)
                        .HasColumnType("numeric(18,6)");

                    b.Property<decimal>("PlannedHectares")
                        .HasPrecision(18, 4)
                        .HasColumnType("numeric(18,4)");

                    b.Property<decimal>("PlannedTotal")
                        .HasPrecision(18, 4)
                        .HasColumnType("numeric(18,4)");

                    b.Property<decimal?>("RealDose")
                        .HasPrecision(18, 6)
                        .HasColumnType("numeric(18,6)");

                    b.Property<decimal?>("RealHectares")
                        .HasPrecision(18, 4)
                        .HasColumnType("numeric(18,4)");

                    b.Property<decimal?>("RealTotal")
                        .HasPrecision(18, 4)
                        .HasColumnType("numeric(18,4)");

                    b.Property<Guid?>("SupplierContactId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("SupplyId")
                        .HasColumnType("uuid");

                    b.Property<int>("TankMixOrder")
                        .HasColumnType("integer");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<string>("UnitOfMeasure")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.HasKey("Id");

                    b.HasIndex("LaborId");

                    b.HasIndex("SupplierContactId");

                    b.HasIndex("SupplyId");

                    b.ToTable("LaborSupplies", "public");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.LaborType", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Description")
                        .HasColumnType("text");

                    b.Property<int?>("ExecutionMode")
                        .HasColumnType("integer");

                    b.Property<string>("ExternalErpId")
                        .HasColumnType("text");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.ToTable("LaborTypes");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.LaborTypeAlias", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("LaborTypeId")
                        .HasColumnType("uuid");

                    b.Property<string>("NormalizedName")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("RawName")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("LaborTypeId");

                    b.ToTable("LaborTypeAliases");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.Lot", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<decimal>("CadastralArea")
                        .HasPrecision(18, 4)
                        .HasColumnType("numeric(18,4)");

                    b.Property<long?>("CodCentro")
                        .HasColumnType("bigint");

                    b.Property<string>("ExternalErpId")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<Guid>("FieldId")
                        .HasColumnType("uuid");

                    b.Property<Geometry>("Geometry")
                        .HasColumnType("geometry(Geometry, 4326)");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("FieldId");

                    b.HasIndex("Geometry");

                    NpgsqlIndexBuilderExtensions.HasMethod(b.HasIndex("Geometry"), "GIST");

                    b.HasIndex("TenantId", "ExternalErpId")
                        .IsUnique()
                        .HasFilter("\"ExternalErpId\" IS NOT NULL");

                    b.ToTable("Lots", "public");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.PaseImputacion", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<decimal>("Cantidad")
                        .HasColumnType("numeric");

                    b.Property<decimal?>("CantidadAuxiliar")
                        .HasColumnType("numeric");

                    b.Property<long>("CodComprobante")
                        .HasColumnType("bigint");

                    b.Property<long>("CodConcepto")
                        .HasColumnType("bigint");

                    b.Property<long?>("CodCuentaDebeAuxiliar")
                        .HasColumnType("bigint");

                    b.Property<long?>("CodCuentaDebeCentro")
                        .HasColumnType("bigint");

                    b.Property<long?>("CodCuentaDebeContabilidad")
                        .HasColumnType("bigint");

                    b.Property<long?>("CodCuentaDebeGestion")
                        .HasColumnType("bigint");

                    b.Property<long?>("CodCuentaHaberAuxiliar")
                        .HasColumnType("bigint");

                    b.Property<long?>("CodCuentaHaberCentro")
                        .HasColumnType("bigint");

                    b.Property<long?>("CodCuentaHaberContabilidad")
                        .HasColumnType("bigint");

                    b.Property<long?>("CodCuentaHaberGestion")
                        .HasColumnType("bigint");

                    b.Property<long>("CodEmpresa")
                        .HasColumnType("bigint");

                    b.Property<long?>("CodListaDePrecios")
                        .HasColumnType("bigint");

                    b.Property<long>("CodMoneda")
                        .HasColumnType("bigint");

                    b.Property<long?>("CodPerfilImputacionDebe")
                        .HasColumnType("bigint");

                    b.Property<long?>("CodPerfilImputacionHaber")
                        .HasColumnType("bigint");

                    b.Property<long?>("CodPersona")
                        .HasColumnType("bigint");

                    b.Property<string>("CodigoConcepto")
                        .HasColumnType("text");

                    b.Property<DateTime>("Fecha")
                        .HasColumnType("timestamp with time zone");

                    b.Property<int>("IdAgrupacionPase")
                        .HasColumnType("integer");

                    b.Property<string>("IdReferencia")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid?>("LaborId")
                        .HasColumnType("uuid");

                    b.Property<bool>("NoImputaAuxiliar")
                        .HasColumnType("boolean");

                    b.Property<bool>("NoImputaCentro")
                        .HasColumnType("boolean");

                    b.Property<bool>("NoImputaContabilidad")
                        .HasColumnType("boolean");

                    b.Property<bool>("NoImputaGestion")
                        .HasColumnType("boolean");

                    b.Property<string>("Notas")
                        .HasColumnType("text");

                    b.Property<int?>("NumeroComprobante")
                        .HasColumnType("integer");

                    b.Property<Guid>("PaseLoteId")
                        .HasColumnType("uuid");

                    b.Property<decimal>("Precio")
                        .HasColumnType("numeric");

                    b.Property<int>("PuntoVenta")
                        .HasColumnType("integer");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("WorkOrderId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("LaborId");

                    b.HasIndex("PaseLoteId");

                    b.HasIndex("WorkOrderId");

                    b.ToTable("PasesImputacion");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.PaseLote", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Descripcion")
                        .HasColumnType("text");

                    b.Property<string>("Estado")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime>("GeneradoEn")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<int>("TotalPases")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.ToTable("PasesLote");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.Rotation", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<Guid>("CampaignLotId")
                        .HasColumnType("uuid");

                    b.Property<DateOnly>("EndDate")
                        .HasColumnType("date");

                    b.Property<Guid>("ErpActivityId")
                        .HasColumnType("uuid");

                    b.Property<string>("Notes")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<DateOnly>("StartDate")
                        .HasColumnType("date");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("CampaignLotId");

                    b.HasIndex("ErpActivityId");

                    b.ToTable("Rotations", "public");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.SharedToken", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<DateTime>("ExpiresAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<bool>("IsRevoked")
                        .HasColumnType("boolean");

                    b.Property<bool>("IsUsed")
                        .HasColumnType("boolean");

                    b.Property<string>("Metadata")
                        .HasColumnType("text");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<string>("TokenHash")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)");

                    b.Property<Guid?>("WorkOrderId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("TokenHash")
                        .IsUnique();

                    b.HasIndex("WorkOrderId");

                    b.ToTable("SharedTokens", "public");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.StrategyItem", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<Guid>("CropStrategyId")
                        .HasColumnType("uuid");

                    b.Property<int>("DayOffset")
                        .HasColumnType("integer");

                    b.Property<string>("DefaultSuppliesJson")
                        .HasColumnType("jsonb")
                        .HasColumnName("DefaultSupplies");

                    b.Property<Guid?>("ErpActivityId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("LaborTypeId")
                        .HasColumnType("uuid");

                    b.Property<int>("SortOrder")
                        .HasColumnType("integer");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("CropStrategyId");

                    b.HasIndex("LaborTypeId");

                    b.ToTable("StrategyItems", "public");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.SupplyAlias", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("NormalizedName")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("RawName")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid>("SupplyId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("SupplyId");

                    b.ToTable("SupplyAliases");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.TankMixRule", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<Guid>("ProductAId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("ProductBId")
                        .HasColumnType("uuid");

                    b.Property<string>("Severity")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasDefaultValue("Warning");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<string>("WarningMessage")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.HasKey("Id");

                    b.HasIndex("ProductAId");

                    b.HasIndex("ProductBId");

                    b.ToTable("TankMixRules", "public");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.Tenant", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("AllowedInventoryGroupsJson")
                        .HasColumnType("text");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<string>("GestorMaxApiKeyEncrypted")
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)");

                    b.Property<string>("GestorMaxDatabaseId")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.HasKey("Id");

                    b.ToTable("Tenants", "public");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.UserProfile", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<string>("DisplayName")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<string>("PasswordHash")
                        .HasColumnType("text");

                    b.Property<string>("PasswordSalt")
                        .HasColumnType("text");

                    b.Property<string>("Role")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasDefaultValue("Agronomist");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.ToTable("UserProfiles", "public");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.WorkOrder", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<bool>("AcceptsMultipleDates")
                        .HasColumnType("boolean");

                    b.Property<bool>("AcceptsMultiplePeople")
                        .HasColumnType("boolean");

                    b.Property<string>("AssignedTo")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<Guid?>("CampaignId")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("ContactId")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("ContractorId")
                        .HasColumnType("uuid");

                    b.Property<string>("Description")
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)");

                    b.Property<DateTime>("DueDate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime>("ExpirationDate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid?>("FieldId")
                        .HasColumnType("uuid");

                    b.Property<bool>("IsExternalBilling")
                        .HasColumnType("boolean");

                    b.Property<Guid?>("LotId")
                        .HasColumnType("uuid");

                    b.Property<string>("Name")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("OTNumber")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime>("PlannedDate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<bool>("StockReserved")
                        .HasColumnType("boolean");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("WorkOrderStatusId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("CampaignId");

                    b.HasIndex("ContactId");

                    b.HasIndex("FieldId");

                    b.HasIndex("LotId");

                    b.HasIndex("WorkOrderStatusId");

                    b.ToTable("WorkOrders", "public");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.WorkOrderStatus", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("AllowedTransitionsJson")
                        .HasColumnType("text");

                    b.Property<string>("ColorHex")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)");

                    b.Property<bool>("IsDefault")
                        .HasColumnType("boolean");

                    b.Property<bool>("IsEditable")
                        .HasColumnType("boolean");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<int>("SortOrder")
                        .HasColumnType("integer");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.ToTable("WorkOrderStatuses", "public");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.WorkOrderSupplyApproval", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<decimal>("ApprovedWithdrawal")
                        .HasPrecision(18, 4)
                        .HasColumnType("numeric(18,4)");

                    b.Property<decimal?>("RealTotalUsed")
                        .HasPrecision(18, 4)
                        .HasColumnType("numeric(18,4)");

                    b.Property<Guid>("SupplyId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<decimal>("TotalCalculated")
                        .HasPrecision(18, 4)
                        .HasColumnType("numeric(18,4)");

                    b.Property<string>("WithdrawalCenter")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<Guid>("WorkOrderId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("SupplyId");

                    b.HasIndex("WorkOrderId");

                    b.ToTable("WorkOrderSupplyApprovals", "public");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.AccountConfiguration", b =>
                {
                    b.HasOne("GestorOT.Domain.Entities.ErpActivity", "ErpActivity")
                        .WithMany()
                        .HasForeignKey("ErpActivityId");

                    b.HasOne("GestorOT.Domain.Entities.LaborType", "LaborType")
                        .WithMany()
                        .HasForeignKey("LaborTypeId");

                    b.Navigation("ErpActivity");

                    b.Navigation("LaborType");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.CampaignField", b =>
                {
                    b.HasOne("GestorOT.Domain.Entities.Campaign", "Campaign")
                        .WithMany("CampaignFields")
                        .HasForeignKey("CampaignId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("GestorOT.Domain.Entities.Field", "Field")
                        .WithMany()
                        .HasForeignKey("FieldId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Campaign");

                    b.Navigation("Field");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.CampaignLot", b =>
                {
                    b.HasOne("GestorOT.Domain.Entities.Campaign", "Campaign")
                        .WithMany("CampaignLots")
                        .HasForeignKey("CampaignId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("GestorOT.Domain.Entities.Lot", "Lot")
                        .WithMany("CampaignLots")
                        .HasForeignKey("LotId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Campaign");

                    b.Navigation("Lot");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.Contact", b =>
                {
                    b.HasOne("GestorOT.Domain.Entities.ErpPerson", "ErpPerson")
                        .WithMany()
                        .HasForeignKey("ErpPersonId");

                    b.Navigation("ErpPerson");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.CropStrategy", b =>
                {
                    b.HasOne("GestorOT.Domain.Entities.ErpActivity", "Activity")
                        .WithMany()
                        .HasForeignKey("ErpActivityId");

                    b.Navigation("Activity");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.Labor", b =>
                {
                    b.HasOne("GestorOT.Domain.Entities.AccountConfiguration", "AccountConfiguration")
                        .WithMany()
                        .HasForeignKey("AccountConfigurationId");

                    b.HasOne("GestorOT.Domain.Entities.CampaignLot", "CampaignLot")
                        .WithMany()
                        .HasForeignKey("CampaignLotId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("GestorOT.Domain.Entities.Contact", "Contact")
                        .WithMany()
                        .HasForeignKey("ContactId");

                    b.HasOne("GestorOT.Domain.Entities.ErpActivity", "ErpActivity")
                        .WithMany()
                        .HasForeignKey("ErpActivityId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("GestorOT.Domain.Entities.LaborType", "Type")
                        .WithMany()
                        .HasForeignKey("LaborTypeId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("GestorOT.Domain.Entities.Lot", "Lot")
                        .WithMany()
                        .HasForeignKey("LotId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("GestorOT.Domain.Entities.Labor", "PlannedLabor")
                        .WithMany()
                        .HasForeignKey("PlannedLaborId");

                    b.HasOne("GestorOT.Domain.Entities.CropStrategy", "SourceStrategy")
                        .WithMany()
                        .HasForeignKey("SourceStrategyId");

                    b.HasOne("GestorOT.Domain.Entities.WorkOrder", "WorkOrder")
                        .WithMany("Labors")
                        .HasForeignKey("WorkOrderId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("AccountConfiguration");

                    b.Navigation("CampaignLot");

                    b.Navigation("Contact");

                    b.Navigation("ErpActivity");

                    b.Navigation("Lot");

                    b.Navigation("PlannedLabor");

                    b.Navigation("SourceStrategy");

                    b.Navigation("Type");

                    b.Navigation("WorkOrder");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.LaborAttachment", b =>
                {
                    b.HasOne("GestorOT.Domain.Entities.Labor", "Labor")
                        .WithMany()
                        .HasForeignKey("LaborId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Labor");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.LaborFileAsset", b =>
                {
                    b.HasOne("GestorOT.Domain.Entities.FileAsset", "FileAsset")
                        .WithMany()
                        .HasForeignKey("FileAssetId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("GestorOT.Domain.Entities.Labor", "Labor")
                        .WithMany()
                        .HasForeignKey("LaborId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("FileAsset");

                    b.Navigation("Labor");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.LaborImportBatch", b =>
                {
                    b.HasOne("GestorOT.Domain.Entities.Campaign", "Campaign")
                        .WithMany()
                        .HasForeignKey("CampaignId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Campaign");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.LaborImportPendingRow", b =>
                {
                    b.HasOne("GestorOT.Domain.Entities.LaborImportBatch", "Batch")
                        .WithMany("Rows")
                        .HasForeignKey("BatchId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Batch");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.LaborSupply", b =>
                {
                    b.HasOne("GestorOT.Domain.Entities.Labor", "Labor")
                        .WithMany("Supplies")
                        .HasForeignKey("LaborId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("GestorOT.Domain.Entities.Contact", "SupplierContact")
                        .WithMany()
                        .HasForeignKey("SupplierContactId");

                    b.HasOne("GestorOT.Domain.Entities.Inventory", "Supply")
                        .WithMany()
                        .HasForeignKey("SupplyId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Labor");

                    b.Navigation("SupplierContact");

                    b.Navigation("Supply");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.LaborTypeAlias", b =>
                {
                    b.HasOne("GestorOT.Domain.Entities.LaborType", "LaborType")
                        .WithMany()
                        .HasForeignKey("LaborTypeId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("LaborType");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.Lot", b =>
                {
                    b.HasOne("GestorOT.Domain.Entities.Field", "Field")
                        .WithMany("Lots")
                        .HasForeignKey("FieldId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Field");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.PaseImputacion", b =>
                {
                    b.HasOne("GestorOT.Domain.Entities.Labor", "Labor")
                        .WithMany()
                        .HasForeignKey("LaborId");

                    b.HasOne("GestorOT.Domain.Entities.PaseLote", "Lote")
                        .WithMany("Pases")
                        .HasForeignKey("PaseLoteId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("GestorOT.Domain.Entities.WorkOrder", "WorkOrder")
                        .WithMany()
                        .HasForeignKey("WorkOrderId");

                    b.Navigation("Labor");

                    b.Navigation("Lote");

                    b.Navigation("WorkOrder");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.Rotation", b =>
                {
                    b.HasOne("GestorOT.Domain.Entities.CampaignLot", "CampaignLot")
                        .WithMany("Rotations")
                        .HasForeignKey("CampaignLotId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("GestorOT.Domain.Entities.ErpActivity", "ErpActivity")
                        .WithMany()
                        .HasForeignKey("ErpActivityId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("CampaignLot");

                    b.Navigation("ErpActivity");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.SharedToken", b =>
                {
                    b.HasOne("GestorOT.Domain.Entities.WorkOrder", "WorkOrder")
                        .WithMany()
                        .HasForeignKey("WorkOrderId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.Navigation("WorkOrder");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.StrategyItem", b =>
                {
                    b.HasOne("GestorOT.Domain.Entities.CropStrategy", "Strategy")
                        .WithMany("Items")
                        .HasForeignKey("CropStrategyId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("GestorOT.Domain.Entities.LaborType", "LaborType")
                        .WithMany()
                        .HasForeignKey("LaborTypeId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("LaborType");

                    b.Navigation("Strategy");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.SupplyAlias", b =>
                {
                    b.HasOne("GestorOT.Domain.Entities.Inventory", "Supply")
                        .WithMany()
                        .HasForeignKey("SupplyId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Supply");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.TankMixRule", b =>
                {
                    b.HasOne("GestorOT.Domain.Entities.Inventory", "ProductA")
                        .WithMany()
                        .HasForeignKey("ProductAId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("GestorOT.Domain.Entities.Inventory", "ProductB")
                        .WithMany()
                        .HasForeignKey("ProductBId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("ProductA");

                    b.Navigation("ProductB");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.WorkOrder", b =>
                {
                    b.HasOne("GestorOT.Domain.Entities.Campaign", "Campaign")
                        .WithMany("WorkOrders")
                        .HasForeignKey("CampaignId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("GestorOT.Domain.Entities.Contact", "Contact")
                        .WithMany()
                        .HasForeignKey("ContactId");

                    b.HasOne("GestorOT.Domain.Entities.Field", "Field")
                        .WithMany("WorkOrders")
                        .HasForeignKey("FieldId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("GestorOT.Domain.Entities.Lot", null)
                        .WithMany("WorkOrders")
                        .HasForeignKey("LotId");

                    b.HasOne("GestorOT.Domain.Entities.WorkOrderStatus", "WorkOrderStatus")
                        .WithMany()
                        .HasForeignKey("WorkOrderStatusId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("Campaign");

                    b.Navigation("Contact");

                    b.Navigation("Field");

                    b.Navigation("WorkOrderStatus");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.WorkOrderSupplyApproval", b =>
                {
                    b.HasOne("GestorOT.Domain.Entities.Inventory", "Supply")
                        .WithMany()
                        .HasForeignKey("SupplyId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("GestorOT.Domain.Entities.WorkOrder", "WorkOrder")
                        .WithMany("SupplyApprovals")
                        .HasForeignKey("WorkOrderId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Supply");

                    b.Navigation("WorkOrder");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.Campaign", b =>
                {
                    b.Navigation("CampaignFields");

                    b.Navigation("CampaignLots");

                    b.Navigation("WorkOrders");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.CampaignLot", b =>
                {
                    b.Navigation("Rotations");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.CropStrategy", b =>
                {
                    b.Navigation("Items");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.Field", b =>
                {
                    b.Navigation("Lots");

                    b.Navigation("WorkOrders");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.Labor", b =>
                {
                    b.Navigation("Supplies");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.LaborImportBatch", b =>
                {
                    b.Navigation("Rows");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.Lot", b =>
                {
                    b.Navigation("CampaignLots");

                    b.Navigation("WorkOrders");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.PaseLote", b =>
                {
                    b.Navigation("Pases");
                });

            modelBuilder.Entity("GestorOT.Domain.Entities.WorkOrder", b =>
                {
                    b.Navigation("Labors");

                    b.Navigation("SupplyApprovals");
                });
#pragma warning restore 612, 618
        }
    }
}
